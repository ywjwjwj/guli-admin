/**
 * tdesign v0.38.1
 * (c) 2022 tdesign
 * @license MIT
 */

import { c as components } from './_chunks/dep-cf6d9bbd.js';
export { Button } from './button/index.js';
export { Divider } from './divider/index.js';
export { Icon } from './icon/index.js';
export { Col, Row } from './grid/index.js';
export { Aside, Content, Footer, Header, Layout } from './layout/index.js';
export { Affix } from './affix/index.js';
export { Anchor, AnchorItem, AnchorTarget } from './anchor/index.js';
export { Breadcrumb, BreadcrumbItem } from './breadcrumb/index.js';
export { Dropdown, DropdownItem, DropdownMenu } from './dropdown/index.js';
export { HeadMenu, Menu, MenuGroup, MenuItem, Submenu } from './menu/index.js';
export { Pagination } from './pagination/index.js';
export { StepItem, Steps } from './steps/index.js';
export { TabPanel, Tabs } from './tabs/index.js';
export { Cascader } from './cascader/index.js';
export { Checkbox, CheckboxGroup } from './checkbox/index.js';
export { DatePicker } from './date-picker/index.js';
export { Form, FormItem } from './form/index.js';
export { Addon, Input, InputGroup } from './input/index.js';
export { InputNumber } from './input-number/index.js';
export { Radio, RadioButton, RadioGroup } from './radio/index.js';
export { Option, OptionGroup, Select } from './select/index.js';
export { Slider } from './slider/index.js';
export { Switch } from './switch/index.js';
export { TagInput } from './tag-input/index.js';
export { SelectInput } from './select-input/index.js';
export { Textarea } from './textarea/index.js';
export { Transfer } from './transfer/index.js';
export { EPickerCols, KeyboardDirection } from './time-picker/interface.js';
export { TimePicker, TimeRangePicker } from './time-picker/index.js';
export { TreeSelect } from './tree-select/index.js';
export { Avatar, AvatarGroup } from './avatar/index.js';
export { Badge } from './badge/index.js';
export { Calendar } from './calendar/index.js';
export { Comment } from './comment/index.js';
export { List, ListItem, ListItemMeta } from './list/index.js';
export { LoadingPlugin } from './loading/plugin.js';
export { Loading } from './loading/index.js';
export { Progress } from './progress/index.js';
export { Skeleton } from './skeleton/index.js';
export { Swiper, SwiperItem } from './swiper/index.js';
export { BaseTable, EnhancedTable, PrimaryTable, Table } from './table/index.js';
export { CheckTag, Tag } from './tag/index.js';
export { Tooltip } from './tooltip/index.js';
export { Tree } from './tree/index.js';
export { Alert } from './alert/index.js';
export { DialogPlugin } from './dialog/plugin.js';
export { Dialog } from './dialog/index.js';
export { Drawer } from './drawer/index.js';
export { MessagePlugin } from './message/plugin.js';
export { Message } from './message/index.js';
export { NotificationPlugin, NotifyPlugin } from './notification/plugin.js';
export { Notification } from './notification/index.js';
export { Popconfirm } from './popconfirm/index.js';
export { Popup } from './popup/index.js';
export { Upload } from './upload/index.js';
export { merge, useConfig, usePrefixClass } from './config-provider/useConfig.js';
export { ConnfigProvider } from './config-provider/index.js';
export { LocaleProvider } from './locale/index.js';
import './button/button.js';
import './_chunks/dep-99305448.js';
import '@babel/runtime/helpers/defineProperty';
import './config.js';
import './utils/classnames.js';
import './button/props.js';
import './utils/render-tnode.js';
import '@babel/runtime/helpers/typeof';
import 'vue';
import 'lodash/isObject';
import './utils/ripple.js';
import './_common/js/utils/set-style.js';
import './config-provider/config-receiver.js';
import 'lodash/mergeWith';
import './config-provider/zh_CN_config.js';
import './config-provider/type.js';
import './utils/mixins.js';
import './loading/loading.js';
import './loading/icon/gradient.js';
import './_common/js/loading/circle-adapter.js';
import './_common/js/utils/helper.js';
import '@babel/runtime/helpers/objectWithoutProperties';
import '@babel/runtime/helpers/slicedToArray';
import './utils/dom.js';
import 'raf';
import 'lodash/isString';
import './utils/easing.js';
import './utils/transfer-dom.js';
import './loading/props.js';
import './utils/withInstall.js';
import 'lodash/capitalize';
import './style/css.js';
import './loading/type.js';
import './button/type.js';
import './divider/divider.js';
import './divider/props.js';
import './divider/type.js';
import 'tdesign-icons-vue';
import './grid/col.js';
import './utils/responsive.js';
import './grid/col-props.js';
import './grid/row.js';
import './grid/row-props.js';
import './grid/type.js';
import './layout/layout.js';
import './layout/header.js';
import './layout/header-props.js';
import './layout/footer.js';
import './layout/footer-props.js';
import './layout/aside.js';
import './layout/aside-props.js';
import './layout/content.js';
import './layout/type.js';
import './affix/affix.js';
import '@babel/runtime/helpers/asyncToGenerator';
import '@babel/runtime/regenerator';
import 'lodash/isFunction';
import './affix/props.js';
import './affix/type.js';
import './anchor/anchor.js';
import './anchor/utils.js';
import './anchor/props.js';
import './anchor/constant.js';
import './anchor/anchor-item.js';
import './anchor/anchor-item-props.js';
import './anchor/anchor-target.js';
import './utils/clipboard.js';
import 'clipboard';
import './anchor/anchor-target-props.js';
import './message/messageList.js';
import './message/const.js';
import './message/message.js';
import './message/props.js';
import './message/animation.js';
import './popup/popup.js';
import '@popperjs/core';
import './utils/helper.js';
import 'lodash/camelCase';
import './popup/props.js';
import './popup/container.js';
import './utils/map-props.js';
import './popup/type.js';
import './anchor/type.js';
import './breadcrumb/breadcrumb.js';
import './breadcrumb/props.js';
import './breadcrumb/breadcrumb-item.js';
import './breadcrumb/breadcrumb-item-props.js';
import './tooltip/tooltip.js';
import './tooltip/props.js';
import './tooltip/type.js';
import './breadcrumb/type.js';
import './dropdown/dropdown-item.js';
import './dropdown/dropdown-item-props.js';
import './utils/event.js';
import './dropdown/dropdown.js';
import './dropdown/dropdown-menu.js';
import './dropdown/props.js';
import '@vue/composition-api';
import './menu/menu.js';
import '@babel/runtime/helpers/toConsumableArray';
import './menu/props.js';
import './menu/v-menu.js';
import '@babel/runtime/helpers/classCallCheck';
import '@babel/runtime/helpers/createClass';
import './menu/head-menu.js';
import './menu/head-menu-props.js';
import './tabs/tabs.js';
import 'lodash/kebabCase';
import './tabs/props.js';
import './tabs/tab-panel.js';
import './tabs/tab-panel-props.js';
import './tabs/tab-nav.js';
import 'lodash/debounce';
import './tabs/tab-nav-item.js';
import './tabs/type.js';
import './menu/submenu.js';
import './menu/submenu-props.js';
import './common-components/fake-arrow.js';
import './menu/menu-item.js';
import './menu/menu-item-props.js';
import './menu/menu-group.js';
import './menu/menu-group-props.js';
import './menu/type.js';
import './pagination/pagination.js';
import './pagination/props.js';
import './input-number/input-number.js';
import './input-number/props.js';
import './input/addon.js';
import './input/input.js';
import './input/props.js';
import './input/input-group.js';
import './input/type.js';
import './input-number/type.js';
import './select/select.js';
import 'lodash/get';
import 'lodash/set';
import './select/option.js';
import './select/option-props.js';
import './checkbox/group.js';
import 'lodash/intersection';
import './checkbox/checkbox.js';
import './checkbox/props.js';
import './checkbox/checkbox-group-props.js';
import './checkbox/type.js';
import './select/props.js';
import './tag/tag.js';
import './tag/props.js';
import './tag/check-tag.js';
import './tag/check-tag-props.js';
import './tag/type.js';
import './select/optionGroup.js';
import './select/option-group-props.js';
import './select/type.js';
import './pagination/type.js';
import './steps/steps.js';
import './steps/props.js';
import './steps/step-item.js';
import './steps/step-item-props.js';
import './steps/type.js';
import './cascader/cascader.js';
import 'lodash/isEqual';
import './_common/js/tree/tree-store.js';
import 'lodash/difference';
import 'lodash/isPlainObject';
import './_common/js/tree/tree-node.js';
import 'lodash/uniqueId';
import './_common/js/tree/tree-node-model.js';
import 'lodash/pick';
import './_common/js/log/log.js';
import './cascader/utils/helper.js';
import 'lodash/isEmpty';
import './cascader/utils/cascader.js';
import './cascader/components/Panel.js';
import './cascader/utils/panel.js';
import 'lodash/isNumber';
import './cascader/components/Item.js';
import './cascader/utils/item.js';
import './cascader/props.js';
import './cascader/components/InputContent.js';
import './cascader/utils/inputContent.js';
import './cascader/interface.js';
import './cascader/type.js';
import './date-picker/date-picker.js';
import 'dayjs';
import './_chunks/dep-e3c17631.js';
import './date-picker/props.js';
import './date-picker/calendar-presets.js';
import './date-picker/panel/date.js';
import './date-picker/basic/header.js';
import './date-picker/basic/table.js';
import './date-picker/basic/cell.js';
import './_common/js/date-picker/utils.js';
import '@babel/runtime/helpers/extends';
import 'lodash/chunk';
import './date-picker/panel/date-range.js';
import './time-picker/panel/index.js';
import './_chunks/dep-ab4703df.js';
import './time-picker/panel/props.js';
import './time-picker/props.js';
import './time-picker/type.js';
import './time-picker/constant.js';
import './date-picker/interface.js';
import './date-picker/type.js';
import './form/form.js';
import './form/props.js';
import './form/const.js';
import './form/form-item.js';
import 'lodash/cloneDeep';
import 'lodash/isNil';
import 'lodash/template';
import './form/form-model.js';
import './form/form-item-props.js';
import './form/type.js';
import './radio/radio.js';
import './radio/props.js';
import './radio/group.js';
import './radio/radio-group-props.js';
import './radio/radio-button.js';
import './radio/type.js';
import './slider/slider.js';
import './slider/props.js';
import './slider/slider-mark.js';
import './slider/slider-button.js';
import './switch/switch.js';
import './switch/props.js';
import './switch/type.js';
import './tag-input/tag-input.js';
import './tag-input/props.js';
import './tag-input/useTagScroll.js';
import './tag-input/useTagList.js';
import './hooks/useVModel.js';
import './hooks/tnode.js';
import './tag-input/useHover.js';
import './hooks/useDefaultValue.js';
import './tag-input/type.js';
import './select-input/select-input.js';
import './select-input/props.js';
import './select-input/useSingle.js';
import './config-provider/config-provider.js';
import './config-provider/context.js';
import './select-input/useMultiple.js';
import './select-input/useOverlayStyle.js';
import './select-input/type.js';
import './textarea/textarea.js';
import './textarea/props.js';
import './textarea/calcTextareaHeight.js';
import './textarea/type.js';
import './transfer/transfer.js';
import './transfer/components/transfer-list.js';
import './transfer/utils.js';
import './transfer/components/transfer-search.js';
import './transfer/components/transfer-operations.js';
import './transfer/props.js';
import './time-picker/time-picker.js';
import './time-picker/input-items.js';
import './time-picker/time-range-picker.js';
import './time-picker/time-range-picker-props.js';
import './tree-select/tree-select.js';
import 'lodash/isArray';
import 'lodash/isBoolean';
import './tree-select/props.js';
import './tree/td-tree.js';
import 'lodash/upperFirst';
import './tree/tree-item.js';
import './tree/util.js';
import './tree/constants.js';
import './tree/props.js';
import './tree/interface.js';
import './tree/type.js';
import './tree-select/interface.js';
import './tree-select/type.js';
import './avatar/avatar.js';
import './avatar/props.js';
import './avatar/group.js';
import './avatar/avatar-group-props.js';
import './avatar/type.js';
import './badge/badge.js';
import './badge/props.js';
import './badge/type.js';
import './calendar/calendar.js';
import './calendar/props.js';
import './calendar/utils.js';
import './calendar/const.js';
import './calendar/calendar-cell.js';
import './utils/render-component.js';
import './calendar/interface.js';
import './calendar/type.js';
import './comment/comment.js';
import './comment/props.js';
import './comment/type.js';
import './list/list.js';
import './list/props.js';
import './list/const.js';
import './list/list-item.js';
import './list/list-item-props.js';
import './list/list-item-meta.js';
import './list/list-item-meta-props.js';
import './list/type.js';
import './progress/progress.js';
import './progress/constants.js';
import './progress/props.js';
import './progress/type.js';
import './skeleton/skeleton.js';
import './skeleton/props.js';
import './skeleton/type.js';
import './swiper/swiper.js';
import './swiper/props.js';
import './swiper/swiper-item.js';
import './swiper/type.js';
import './table/base-table/index.js';
import 'lodash/throttle';
import './table/util/props-util.js';
import './table/base-table-props.js';
import './table/base-table/table-body.js';
import './table/primary-table-props.js';
import './table/base-table/table-row.js';
import './table/base-table/table-cell.js';
import './table/util/common.js';
import './table/base-table/table-header.js';
import './table/base-table/col-group.js';
import './table/util/interface.js';
import './table/primary-table/index.js';
import './table/primary-table/mixins/expand.js';
import './table/primary-table/expand-box.js';
import './table/primary-table/mixins/select.js';
import './table/primary-table/select-box.js';
import './table/primary-table/mixins/sort.js';
import './table/primary-table/sorter-button.js';
import './table/primary-table/mixins/row-draggable.js';
import './table/primary-table/mixins/filter.js';
import './table/primary-table/mixins/show-columns.js';
import './dialog/dialog.js';
import './dialog/actions.js';
import './dialog/props.js';
import './table/primary-table/mixins/async-loading.js';
import './table/enhanced-table/index.js';
import './table/enhanced-table-props.js';
import './table/enhanced-table/tree.js';
import './table/enhanced-table/tree-store.js';
import './table/enhanced-table/tree-select.js';
import './table/type.js';
import './alert/alert.js';
import './alert/props.js';
import './alert/type.js';
import './dialog/type.js';
import './drawer/drawer.js';
import './drawer/props.js';
import './drawer/type.js';
import './message/type.js';
import './notification/notificationList.js';
import './notification/notification.js';
import './notification/props.js';
import './notification/const.js';
import './notification/type.js';
import './popconfirm/popconfirm.js';
import './popconfirm/props.js';
import './popconfirm/type.js';
import './upload/upload.js';
import 'lodash/findIndex';
import 'lodash/without';
import './upload/dragger.js';
import './upload/util.js';
import 'lodash/uniqWith';
import './upload/props.js';
import './upload/image.js';
import './upload/flow-list.js';
import './_common/js/upload/xhr.js';
import './_common/js/log/index.js';
import './upload/single-file.js';
import './upload/interface.js';
import './upload/type.js';
import './locale/local-provider.js';
import './locale/type.js';

function install(Vue, config) {
  Object.keys(components).forEach(function (key) {
    if (components[key]) {
      /plugin/i.test(key) ? Vue.use(components[key]) : Vue.use(components[key], config);
    }
  });
}

if (typeof window !== "undefined" && window.Vue) {
  install(window.Vue);
}
var index = {
  install: install,
  version: typeof "0.38.1" === "undefined" ? "" : "0.38.1"
};

export { index as default };
//# sourceMappingURL=index.js.map
